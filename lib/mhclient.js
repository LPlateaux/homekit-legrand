...
class MyHomeClient {
    ...
    simpleBlindCommand(_address,_targetPosition,_time,_caller) {
        var address = this._slashesToAddress(_address);
		var travelTimeMs ;
...
//		buffer de commandes pour éviter que l'application envoie des commandes multiples
		if (this.blindBuffers[sprintf("Blind-%s",_address)] != null) {
			_caller.log.info(sprintf("Nouvelle commande à %s pour le volet %s, je stoppe le TimeOut", _targetPosition, address));
			clearTimeout(this.blindBuffers[sprintf("Blind-%s",_address)]);
		}
		this.blindBuffers[sprintf("Blind-%s",_address)] = setTimeout(function() {
			_caller.log.info(sprintf("Traitement de la commande à %s pour le volet %s", _targetPosition, address));
...
//			targetPosition = 100 qd levé et =0 qd baissé 			
			...
			if (_time != 0) {
				_caller.log.info(sprintf("Commande avec temps de parcours %s pour le volet %s", _time, address));
				/*Si complétement ouvert*/
				if (_targetPosition > 95) {
					this.command.send(sprintf("*2*%d*%s##",1,address));
					_caller.log.info(sprintf("Demande position ouverte complète, envoi de la commande 1 pour le volet %s", address));
				}
				/*Si complétement fermé*/
				if (_targetPosition < 5 ) {
					this.command.send(sprintf("*2*%d*%s##",2,address));
					_caller.log.info(sprintf("Demande position fermée complète, envoi de la commande 2 pour le volet %s", address));
				}
				/* Si entrouvert en bas */
				if (_targetPosition >= 5 && _targetPosition <= 50) {
					_caller.log.info(sprintf("Demande position fermeture partielle à %s, pour le volet %s", _targetPosition, address));
					travelTimeMs = _time*1000*(_targetPosition)/100;
					_caller.log.info(sprintf("***Calcul du temps de remontée : %s ms", travelTimeMs));
					this.command.send(sprintf("*2*%d*%s##",2,address));
					_caller.log.info(sprintf("***Ordre de descente pour calage"));
					_caller.log.info(sprintf("***Attente fin de descente"));
					setTimeout(function goUp() {
							_caller.log.info(sprintf("***Descente terminée"));
							this.command.send(sprintf("*2*%d*%s##",1,address));
							_caller.log.info(sprintf("***Début remontée pendant %s ms", travelTimeMs));
							setTimeout(function() {
								this.command.send(sprintf("*2*%d*%s##",0,address));
								_caller.log.info(sprintf("***Fin remontée"));									
							}.bind(this), travelTimeMs);
					}.bind(this), _time*1000*1.1);
				}
				/* Si entrouvert en haut */
				if (_targetPosition > 50 && _targetPosition <= 95) {
					_caller.log.info(sprintf("Demande position fermeture partielle à %s pour le volet %s", _targetPosition, address));
					travelTimeMs = _time*1000*(100-_targetPosition)/100;
					_caller.log.info(sprintf("***Calcul du temps de redescente : %s ms", travelTimeMs));
					this.command.send(sprintf("*2*%d*%s##",1,address));
					_caller.log.info(sprintf("***Ordre de montée pour calage"));
					_caller.log.info(sprintf("***Attente fin de montée"));
					setTimeout(function goDown() {
							_caller.log.info(sprintf("***Montée terminée"));
							this.command.send(sprintf("*2*%d*%s##",2,address));
							_caller.log.info(sprintf("***Début redescente pendant %s ms", travelTimeMs));
							setTimeout(function() {
								this.command.send(sprintf("*2*%d*%s##",0,address));
								_caller.log.info(sprintf("***Fin de redescente"));
							}.bind(this), travelTimeMs);
					}.bind(this), _time*1000*1.1);
				}
			} else {
				_caller.log.info(sprintf("Commande SANS temps de parcours pour le volet %s", _time, address));
				if (_targetPosition > 95) {
//					this.mh.simpleBlindCommand(this.address,1);
					this.command.send(sprintf("*2*%d*%s##",1,address));
					_caller.log.info(sprintf("***Commande à monter pour le volet %s", address));
				}
				if (_targetPosition < 5 ) {
//					this.mh.simpleBlindCommand(this.address,2);
					this.command.send(sprintf("*2*%d*%s##",2,address));
					_caller.log.info(sprintf("***Commande à descendre pour le volet %s", address));
				}
				if (_targetPosition >=5 && _targetPosition <=95) {
//					this.mh.simpleBlindCommand(this.address,0);
					this.command.send(sprintf("*2*%d*%s##",0,address));
					_caller.log.info(sprintf("***Commande à arrêt pour le volet %s", address));
				}
			}				
			delete(this.blindBuffers[sprintf("Blind-%s", _address)]);
		}.bind(this), 2000);
		_caller.log.info(sprintf("J'ai rempli le buffer blindBuffers[ %s ] avec un id %s",sprintf("Blind-%s",_address), this.blindBuffers[sprintf("Blind-%s",_address)]));
